...
class DeleteTricot extends StatelessWidget {
  const DeleteTricot({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocListener<
        GenericActionBloc<TricotDto>,
        BaseActionState<TricotDto>
    >(
      listener: (context, state) {
        if (state is ActionSuccessState) {
          ...
          context.read<GenericListBloc<TricotDto>>().add(LoadEvent());
        } else if (state is ActionFailureState<TricotDto>) {
      ...
      child: BlocBuilder<
          GenericListBloc<TricotDto>,
          BaseListState<TricotDto>
      >(
  ...
  Future<void> _handleDelete(BuildContext context, String id) async {
    ...
    final interactor = getIt<TricotInteractor>();
    if (confirm) {
      try {
